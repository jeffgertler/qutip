...
drive_term = (a**2 - 1j*b**2)

#H = drive * a **2 + drive.conjugate() * (a**2).dag()
H = drive * drive_term + drive.conjugate() * drive_term.dag()
...
#                        options=opts, progress_bar = True)
    result = qt.mesolve(H, psi0, times, [joint_loss * drive_term, loss * a, loss * b], 
                        [a.dag() * a, b.dag() * b], 
                        options=opts, progress_bar = True)
    print('solved!')
