...
def make_plots(plot_num, num_steps, times, states, psi0, psi_final, N, plot_filepath):
    ...
    num_points = 30
    xvec = np.linspace(-6, 6, num_points)
    t_start = time.time()
    ...
    for n in range(num_steps):
        ...
        Q = qfunc_rere(N, states[n].full(), xvec)
    ...
    pl.savefig(plot_filepath + 'ReRe_qfunc' + str(plot_num) + '.png')
    pl.clf()
...
qutip_filepath = ''
plot_filepath = qutip_filepath + 'out/better_ss/' + ''.join(date) + '/'
